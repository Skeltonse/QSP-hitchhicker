...
inst_tol=10**(-12)
pathname="random_lowsparsity_cases.py"
...
##Can be set over range (0, 10) to show more instances
for j in range(0, 1):
    with open(os.path.join(save_path, "random_decspar"+str(indexstringarray[j])+"_benchmark_data.csv"), 'rb') as f:        allinstsdict=pickle.load(f)
...
for j in range(failcount):
    ...
    nonzeronuma=np.where(abs(a)>10**(-12))[0]
    nonzeronumb=np.where(abs(b)>10**(-12))[0]
    
    numcoeffs=np.unique(np.concatenate((nonzeronuma, nonzeronumb), 0))
    nfailarry=np.append(nfailarry, len(numcoeffs))
...
plt.xlabel("degree")
plt.ylabel("norm")
#tikzplotlib.save(os.path.join(thesispath, "sparfailsuborm2comp.tex"), flavor="context")
plt.show()
...
for j in range(succcount):
    ...
    print('num non zero', len(np.where(abs(a)>10**(-13))[0]))
    nonzeronuma=np.where(abs(a)>10**(-12))[0]
    nonzeronumb=np.where(abs(b)>10**(-12))[0]

    numcoeffs=np.unique(np.concatenate((nonzeronuma, nonzeronumb), 0))
    nsuccarry=np.append(nsuccarry,len(numcoeffs))
...
plt.xlabel("ratio of non-zero coefficients")
tikzplotlib.save(os.path.join(thesispath, "sparsefaildegreelineplot.tex"), flavor="context")
plt.show()

plt.scatter(nfailarry/(2*nz+1), normfailarry)
plt.scatter(nsuccarry/(2*nz+1), normsuccarry)
